...
class RemoteEmergenceMedia extends EmergenceMedia
{
	...
	static public function createFromFile($file, $fieldValues = array())
	{
		...
		$url = static::$remoteStore . '/upload' . '?Key=' . static::$remoteKey;
		
		if(count($fieldValues)>0)
		{
			$url .= '&data=' . json_encode($fieldValues);
		}
		...
		if(is_array($result))
		{
			...
			{
				throw new Exception('The media was not created.');
			}
		}
		else
		{
   ...
   static public function getRecordByField($field, $value, $cacheIndex = false)
    ...
    {
        $URL = static::$remoteStore . '/delete/' . $id . '?Key=' . static::$remoteKey;
        ...
        {
            throw new Exception('Destroying remote record failed.');
        }
    }
